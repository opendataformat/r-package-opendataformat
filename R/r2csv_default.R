#' @title RData to CSV for the default language
#'
#' @description Functions that are used to convert an R data frame
#' to the internal Open Data Format (CSV files) if the "default" language is selected.
#'
#' @import utils
#'
...
#'
#' @param languages The language option is set to "default".
#'
#' @param variables Choose "yes" if all data set variables should be
#' converted, or "no" if not.
#'
#' @param export_data Choose "yes" if the data set CSV should be copied to the
#' output directory, or "no" if not.
#'
#' @return CSV files holding the data and the metadata for the default language.
#'
# --------------------
# dataset.csv with DATA FRAME INPUT
# --------------------
#' @noRd
get_dataset_default <- function(data) {
        header <- NULL
        metadata <- NULL
        # - data set name
        ...
        # - data set label
        if ("label" %in% names(attributes(data))) {
          header <- append(header, "label")
          metadata <- append(metadata, attributes(data)$label)
        }
        # - data set description
        if ("description" %in% names(attributes(data))) {
          header <- append(header, "description")
          metadata <- append(metadata, attributes(data)$description)
        }
        # - data set url
        ...
        # - make dataframe for data set information
...
#' variables header
#' @noRd
var_header_data_default <- function(data) {
    var_header <- c()
    ...
    # - drop labels, label_, description_ attributes (categories.csv)
...
#' variables matrix
#' @noRd
var_matrix_default <- function(data) {
    # - make matrix
    var_mat <- matrix(
        nrow = length(attributes(data)$names), # num variables
        ncol = length(var_header_data_default(data)), # num header items
        dimnames = list(
            attributes(data)$names, # variable names from data
            var_header_data_default(data) # header items
            )
        )
    return(var_mat)
}
...
#' @noRd
var_variable_column_default <- function(data) {
    column <- NULL
    var_id <- 1
...
#' @noRd
var_df_default <- function(data) {
    # - make data frame from matrix
    var_df <- data.frame(var_matrix_default(data), row.names = NULL)
    # - if column "variable" exists
    if ("variable" %in% var_header_data_default(data) == TRUE) {
        var_df[["variable"]] <- var_variable_column_default(data)
    }
    # - iterate through header to fill each column with metadata
    for (item in var_header_data_default(data)) {
        if (item != "variable") {
            column <- NULL
            for (var in attributes(data)$names) {
...
#' variables header
#' @noRd
var_header_var_default <- function(variable) {
    var_header <- c()
...
#' variables matrix
#' @noRd
var_matrix_var_default <- function(variable) {
    # - make matrix
    var_mat <- matrix(
        nrow = 1, # num variables
        ncol = length(var_header_var_default(variable)), # num header items
        dimnames = list(
            attributes(variable)$name, # variable name
            var_header_var_default(variable) # header items
        )
    )
    return(var_mat)
}
...
#' @noRd
var_variable_column_var_default <- function(variable) {
    column <- NULL
    # - if variable has "name" attribute
...
#' making variables data frame
#' @noRd
var_df_var_default <- function(variable) {
    # - make data frame from matrix
    var_df <- data.frame(var_matrix_var_default(variable), row.names = NULL)
    # - if column "variable exists
    if ("variable" %in% var_header_var_default(variable) == TRUE) {
        var_df[["variable"]] <- var_variable_column_var_default(variable)
    }
    # - iterate through header to fill each column with metadata
    for (item in var_header_var_default(variable)) {
        if (item != "variable") {
            column <- NULL
            # if variable has attribute
...
#' categories header
#' @noRd
cat_header_data_default <- function(data) {
    cat_header <- NULL
...
cat_variable_column_data_default <- function(data) {
    ...
    for (var in names(data)) {
        ...
        if ("labels" %in% names(attributes(data[[var]]))) {
            ...
            if (is.null(attributes(data[[var]])$name) == TRUE) {
              var_names <- append(
                var_names,
                rep(var,
                    length(attributes(data[[var]])$labels)))
            }
            if (is.null(attributes(data[[var]])$name) == FALSE) {
              if (attributes(data[[var]])$name == "") {
                  var_names <- append(
                      var_names,
                      rep(var,
...
#' categories matrix
#' @noRd
cat_matrix_data_default <- function(data) {
    ...
    cat_mat <- matrix(
        ...
        ncol = length(cat_header_data_default(data)), # num header items
        dimnames = list(
            cat_variable_column_data_default(data), # variable name
            cat_header_data_default(data) # header items
        )
    )
    return(cat_mat)
}
#' making column "value"
#' @noRd
cat_values_column_data_default <- function(data) {
  values <- NULL
  for (var in attributes(data)$names) {
    # -- is variable categorical?
    if ("labels" %in% names(attributes(data[[var]]))) {
...
#' @noRd
cat_labels_column_data_default <- function(data, item) {
  labels <- NULL
  for (var in attributes(data)$names) {
...
#' @noRd
cat_df_data_default <- function(data) {
  # - make data frame from matrix
  cat_df <- data.frame(cat_matrix_data_default(data), row.names = NULL)
  # - append columns
  for (item in cat_header_data_default(data)) {
    # -- column "variable"
    if (item == "variable") {
      cat_df[["variable"]] <- cat_variable_column_data_default(data)
    }
    # -- column "value
    if (item == "value") {
      cat_df[["value"]] <- cat_values_column_data_default(data)
    }
    # -- column "labels"
    if (startsWith(item, "labels") == TRUE) {
      cat_df[[item]] <- cat_labels_column_data_default(data, item)
    }
  }
  return(cat_df)
}
...
#' categories header
#' @noRd
cat_header_var_default <- function(variable) {
  cat_header <- NULL
...
#' making column "variable"
#' @noRd
cat_variable_column_var_default <- function(variable) {
  var_names <- NULL
  # - is variable categorical?
...
#' categories matrix
#' @noRd
cat_matrix_var_default <- function(variable) {
  ...
  if ("labels" %in% names(attributes(variable))) {
    ...
    cat_mat <- matrix(
      ...
      ncol = length(cat_header_var_default(variable)), # num header items
      dimnames = list(
        cat_variable_column_var_default(variable), # variable name
        cat_header_var_default(variable) # header items
      )
    )
    return(cat_mat)
    } else {
...
#' making column "value"
#' @noRd
cat_values_column_var_default <- function(variable) {
  values <- NULL
  # - is variable categorical?
  if ("labels" %in% names(attributes(variable))) {
...
#' making categories data frame
#' @noRd
cat_df_var_default <- function(variable) {
  # - make data frame from matrix
  cat_df <- data.frame(cat_matrix_var_default(variable), row.names = NULL)
  # - append columns
  for (item in cat_header_var_default(variable)) {
    # -- column "variable"
    if (item == "variable") {
      cat_df[["variable"]] <- cat_variable_column_var_default(variable)
    }
    # -- column "value
    if (item == "value") {
      cat_df[["value"]] <- cat_values_column_var_default(variable)
    }
    # -- columns "labels", "labels_de", ...
    if (startsWith(item, "labels") == TRUE) {
      cat_df[[item]] <- cat_labels_column_var_default(variable, item)
    }
  }
  return(cat_df)
}
  ...
  if ("data.frame" %in% class(input) == TRUE) {
      ...
      if (variables == "yes") {
          write_odf_csv(
              ...
              output)
          write_odf_csv(
              var_df_default(input),
              "/variables.csv",
              output)
          cat_csv<-cat_df_data_default(input)
