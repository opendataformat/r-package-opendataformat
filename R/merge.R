...
merge.opendf<-function(x, y, 
  ...
  if (is.null(by.x) & is.null(by.y)){
    ...
    if (is.null(by)){
      data_out <- merge.data.table(x = x, y = y, by.x = by.x, by.y = by.y, 
                                   all = all, all.x = all.x, all.y = all.y, 
                                 allow.cartesian = allow.cartesian, 
                                 incomparables = incomparables)
    } else {
      data_out <- merge.data.table(x = x, y = y, by = by, by.x = by.x, by.y = by.y, 
                                   all = all, all.x = all.x, all.y = all.y, 
                                   allow.cartesian = allow.cartesian, 
                                   incomparables = incomparables)
    }
    ...
    if (var %in% colnames(x)){
      ...
      if (var %in% colnames(y)){
        ...
        if (var %in% paste0(colnames(x), suffixes[1])){
          attributes(data_out[var]) <- attributes(y[gsub(suffixes[1], "", var)])
        }
        if (var %in% paste0(colnames(y), suffixes[2])){
          attributes(data_out[var]) <- attributes(y[gsub(suffixes[2], "", var)])
        }
      }
    }
  }
