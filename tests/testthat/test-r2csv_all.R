...
test_that("var_matrix", {
  ...
  expect_equal(ncol(var_matrix(df)), 9)
  # test number of rows
...
test_that("var_df_label", {
  ...
  # -- test
  expect_equal(var_df(df_no_label)[["label"]],
               c("",
                 "",
                 "Pressed For Time Last 4 Weeks",
                 "Run-down, Melancholy Last 4 Weeks",
  ...
  expect_equal(colnames(var_df(df_no_label)),
               c("variable", "label_en", "label_de", "description",
...
test_that("var_header_var", {
  ...
  expect_equal(var_header_var(df$bap87),
               c("variable", "label", "label_en", "label_de", "description",
                 "description_en", "description_de", "type", "url" ))
...
test_that("var_matrix_var", {
  ...
  df <- get(load("testdata/data_odf.RData"))
  # -- test
  expect_equal(ncol(var_matrix_var(df$bap87)), 9)
  ...
  attributes(df$bap87)$url <- NULL
  # -- test
  expect_equal(ncol(var_matrix_var(df$bap87)), 7)
  expect_equal(nrow(var_matrix_var(df$bap87)), 1)
})
...
test_that("cat_header_data", {
  ...
  df <- get(load("testdata/data_odf.RData"))
  # -- test
  expect_equal(cat_header_data(df), c("variable", "value", "labels",
  "labels_en", "labels_de"))
})
...
test_that("cat_variable_column_data", {
  ...
  expect_equal(length(cat_variable_column_data(df_empty_labels)), 28)
  # -- manipulate data: no labels
  df_no_labels <- df
  attributes(df_no_labels$bap9001)$labels <- NULL
  expect_equal(length(cat_variable_column_data(df_no_labels)), 27)
})
...
test_that("cat_matrix_data", {
  ...
  expect_equal(ncol(cat_matrix_data(df)), 5) # ncol
...
test_that("cat_labels_column_data", {
  ...
  df <- get(load("testdata/data_odf.RData"))
  # --- test
  expect_equal(cat_labels_column_data(df, "labels"), c(
  ...
  attributes(df$bap96)$labels <- NULL
  # --- test
  expect_equal(length(cat_labels_column_data(df, "labels")), 32)
  # -- manipulate data: another variable is categorical but labels are empty
  attributes(df$bap87)$labels <- ""
  # --- test
  expect_equal(length(cat_labels_column_data(df, "labels")), 26)
})
...
test_that("cat_header_var", {
  ...
  expect_equal(cat_header_var(df$bap87), c(
    "variable", "value", "labels", "labels_en", "labels_de")
  )
  ...
  attributes(df$bap87)$labels <- NULL
  print(attributes(df$bap87)$labels)
  # --- test
  expect_equal(cat_header_var(df$bap87), c(
...
test_that("cat_matrix_var", {
  ...
  expect_equal(ncol(cat_matrix_var(df$bap96)), 5)
  # -- manipulate data: empty labels
...
test_that("cat_labels_column_var'", {
    ...
    df <- get(load("testdata/data_odf.RData"))
    # --- test
    expect_equal(cat_labels_column_var(df$bap87, "labels"), c(
    ...
    attributes(df$bap87)$labels <- ""
    # --- test
    expect_equal(cat_labels_column_var(df$bap87, "labels"), "")
    # -- manipulate data: no labels
    attributes(df$bap87)$labels <- NULL
    # --- test
    expect_equal(cat_labels_column_var(df$bap87, "labels"), NULL)
...
test_that("cat_df_var", {
  ...
  attributes(df$bap96)$labels <- ""
  expect_equal(cat_df_var(df$bap96)[["labels"]], "")
