...
By default, when using the `docu_opendf()` function, dataset-level information is presented through an HTML page. If you're utilizing RStudio, this page will be displayed within the RStudio viewer. 
...
docu_opendf(df$bap9001, languages = "all", variables="yes")
```
...
Alternatively you can set the current or active language for a dataset. (This function tries to copy the label language function from stata.)
...
```{r labels_opendf1, eval = FALSE, comment = ""}
labels_opendf(df)
```
...
```{r labels_opendf2, eval = FALSE, comment = ""}
...
attributes(df$bap87)$description<-NULL
attributes(df$bap87)$description
```


As shown before, you can also use the labels_opendf() function to retrieve labels and other metadata for the variables:

```{r labels_opendf3, eval = FALSE, comment = ""}
labels_opendf(df)
```

or for a specific variable::

```{r labels_opendf4, eval = FALSE, comment = ""}
...
labels_opendf(df, languages="en")
```

Or you set the active language of the dataset using the setLanguage_opendf() function:
...
You can use the labels_opendf function also to retrieve value labels for a specific variable:

```{r labels_opendf valuelabels, eval = FALSE, comment = ""}
labels_opendf(df$bap9001, valuelabels=T)
```
The value labels for each value are stored in the namespace:

```{r labels_opendf valuelabels names, eval = FALSE, comment = ""}
names(labels_opendf(df$bap9001, valuelabels=T))
```


You can also retrieve variable description(s):

```{r labels_opendf descriptions, eval = FALSE, comment = ""}
names(labels_opendf(df, retrieve="description"))
```

the variable url(s):

```{r labels_opendf url, eval = FALSE, comment = ""}
names(labels_opendf(df, retrieve="url"))
```

and the variable type(s):

```{r labels_opendf type, eval = FALSE, comment = ""}
names(labels_opendf(df, retrieve="type"))
```
...
write_opendf(
  x=df_rec, 
  file = "../data_bap87_rec_metadata.zip",
  export_data = "no"
)
```
...
df_de<- read_opendf(file = path, languages = "de")
```

or

```{r read_opendf choice de, eval = FALSE, comment = ""}
df_de <- read_opendf(file = "../my_data.zip", languages = "de")
```
